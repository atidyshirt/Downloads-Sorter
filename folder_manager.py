...
def move_files(current_dir: 'Path', folder_name: 'str'):
    ...
    '''
    folder1 = ''  # Set the paths for these variables, and use the corrosponding extention in the places below
    folder2 = ''
    folder3 = ''

    for file in current_dir.glob("*."+folder_name):
        if folder_name == '': # Set extention1
            shutil.move(file.name, folder1)
        elif folder_name == '': # Set extention2
            shutil.move(file.name, folder2)
        elif folder_name == '': # Set extention3
            shutil.move(file.name, folder3)
...
def del_folder_redirect(current_dir: 'Path', file_types: 'list'):
    ...
    for root, dirnames, filenames  in os.walk(current_dir):
        ...
        for directory in directorys:
            for i in directory:
                """
                    In the of below set the extentions
                    again
                """
                if i == '' or i == '' or i == '':
                    os.rmdir(i)
